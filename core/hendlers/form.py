...
from core.hendlers.apsched import send_message_middleware
...
async def get_name(message: Message, state: FSMContext):
    ...
    await state.set_state(StepsForm.GET_LAST_NAME) #переводит в ожидании фамилии


async def get_last_name(message:Message, state: FSMContext):
    await message.answer(f'Твоя фамилия:\r\n {message.text}\r\n Теперь введи возраст:')
    await state.update_data(last_name=message.text)
    await state.set_state(StepsForm.GET_AGE) #переводит в ожидании возраста
...
async def get_age(message:Message, bot: Bot, state: FSMContext, apscheduler: AsyncIOScheduler):
    ...
    await message.answer(f'Сохраняем данные: \r\n{str(context_data)}')
